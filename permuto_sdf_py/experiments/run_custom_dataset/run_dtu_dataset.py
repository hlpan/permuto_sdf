...
parser.add_argument('--run_type', default="train", type=str, help='run type: train, mesh')
...
def run():
    ...
    loader_train.set_dataset_path("/workspace/home/nerf-data/data_DTU/")
    # if low_res:
    #     loader_train.set_subsample_factor(4)
    loader_train.set_mode_train()
    loader_train.set_load_mask(False)
    loader_train.set_restrict_to_scene_name("dtu_scan24")
    loader_train.start()
    ...
    if(args.run_type == "train"):
        train(args, config_path, hyperparams, train_params, None, experiment_name, with_viewer, checkpoint_path, tensor_reel, frames_train=loader_train.get_all_frames(), hardcoded_cam_init=False)
